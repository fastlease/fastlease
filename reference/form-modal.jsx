...
function FormModal({ open, onClose, mode }) {
	...
	const estimate = React.useMemo(() => {
		...
		const target =
			parseInt((data.targetRent || "").replace(/[^0-9]/g, ""), 10) || 0;
		const recRent = {
			"Jr 1BR": 2200,
			"1BR": 2650,
			"1BR+den": 2900,
				...
				<div className="fm" onClick={(e) => e.stopPropagation()}>
					<div className="fm-head">
						<div>
							...
							<h3 className="fm-step-h" style={{ marginTop: 8 }}>
								Pick a window that works.
							</h3>
						</div>
						<button aria-label="Close" className="fm-close" onClick={onClose}>
							×
						</button>
					</div>
					<div className="fm-body">
						<p className="fm-step-sub">
							We'll send a short calendar invite. No deck, no pitch — fifteen
							minutes on what you'd want from us, and whether we can deliver it
							for your unit.
						</p>
					...
					<div className="fm-foot">
						<span className="fm-back" style={{ color: "var(--ink-mute)" }}>
			...
			<div className="fm" onClick={(e) => e.stopPropagation()}>
				...
				<div className="fm-progress">
					{Array.from({ length: STEPS }, (_, i) => (
						<div
							className={`fm-progress-step ${i < step ? "done" : i === step ? "active" : ""}`}
							key={i}
						/>
					))}
				</div>
				...
				{step === 0 && (
					<div className="fm-body">
						...
						<p className="fm-step-sub">
							We use building-level comparables, so the neighborhood matters
				...
				{step === 1 && (
					<div className="fm-body">
						...
						<p className="fm-step-sub">
							Bedroom count determines time-to-lease more than any other input.
						</p>
						<div className="fm-field">
							<label className="fm-label">Bedrooms</label>
							<div className="fm-radio-group">
								{["Jr 1BR", "1BR", "1BR+den", "2BR", "2BR+den", "3BR"].map(
									(b) => (
										<button
											className={`fm-radio ${data.bedrooms === b ? "on" : ""}`}
											key={b}
				...
				{step === 2 && (
					<div className="fm-body">
						...
						<p className="fm-step-sub">
							Optional. We'll always work to twenty-one days — this just helps
							us flag anything tighter than that.
						</p>
						<div className="fm-field">
							...
							<div className="fm-radio-group">
								{[
									...
									<button
										className={`fm-radio ${data.move === o.k ? "on" : ""}`}
										key={o.k}
				...
				{step === 3 && (
					<div className="fm-result">
						<div className="fm-eyebrow" style={{ marginBottom: 8 }}>
						...
						<p className="fm-step-sub" style={{ marginBottom: 20 }}>
							Based on comparable units leased in the last 90 days. Numbers
							refresh quarterly.
						</p>
						...
						<div className="fm-result-row">
							<div>
								...
								<div className="fm-result-note" style={{ marginTop: 4 }}>
									Average across recent comparables.
								</div>
							</div>
							<div className="fm-result-val acc">
								{estimate.days}{" "}
								<span style={{ fontSize: 16, color: "var(--ink-mute)" }}>
									days
								</span>
							</div>
						</div>
						<div className="fm-result-row">
							...
							<div className="fm-result-val num">
						...
						{estimate.over > 0 && (
							...
							>
								Heads-up: your target rent is above our suggested range. We can
								still list it — the guarantee terms change above 5% over our
								recommendation.
							</div>
						)}
						...
						<div className="fm-result-cta">
							<button className="btn btn-primary" onClick={onClose}>
								Email me this estimate <span className="arr">→</span>
							</button>
