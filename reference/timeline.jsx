...
function TimelineCalendar() {
  ...
  return (
    <div ref={ref} className="tl-calendar">
      <div className="tl-cal-header">
        <span className="label">Day 01</span>
        <span className="label" style={{ textAlign: 'right' }}>Day 21</span>
      </div>
      <div className="tl-cal-grid">
        {days.map((d) => {
          ...
          return (
            <div key={d} className={`tl-cal-cell ${visible ? 'in' : ''} ${isToday ? 'today' : ''} ${ms ? 'has-ms' : ''}`}>
              <div className="tl-cal-tick" />
              <div className="tl-cal-day num">{d.toString().padStart(2,'0')}</div>
              {ms && (
                <div className={`tl-cal-pin ${ms.phase === 1 ? 'p1' : ms.phase === 2 ? 'p2' : 'p3'}`}>
      ...
      <div className="tl-cal-phases">
        {PHASES.map((p) => {
          ...
          return (
            <div key={p.id} className="tl-cal-phase" style={{ left: `${left}%`, width: `${width}%` }}>
              <div className="tl-cal-phase-bar"/>
              <div className="tl-cal-phase-name">{p.name}</div>
              <div className="tl-cal-phase-range num">Days {a}{a===b?'':`–${b}`}</div>
            </div>
          );
        })}
      </div>
    ...
    <div ref={ref} className="tl-beam">
      <div className="tl-beam-axis">
        {PHASES.map((p) => {
          ...
          const width = ((b - a + 1) / 21) * 100;
          return (
            <div key={p.id} className={`tl-beam-band band-${p.id}`} style={{ left: `${left}%`, width: `${width}%` }}>
        ...
        <div className="tl-beam-today" style={{ left: `${((TODAY - 0.5) / 21) * 100}%` }}>
      ...
      <div className="tl-beam-marks">
        {MILESTONES.map((m, i) => (
          <div key={m.day} className={`tl-beam-mark ${i < shown ? 'in' : ''}`}
...
function TimelineGantt() {
  const [ref, shown] = useSequentialEntry(PHASES.length + MILESTONES.length, 90);
  return (
    <div ref={ref} className="tl-gantt">
      <div className="tl-gantt-grid">
        {Array.from({ length: 21 }, (_, i) => i + 1).map((d) => (
          <div key={d} className="tl-gantt-col">
            {(d === 1 || d === 7 || d === 14 || d === 21) && (
              <span className="tl-gantt-col-lbl num">{d.toString().padStart(2,'0')}</span>
            )}
          </div>
        ))}
      </div>
      <div className="tl-gantt-rows">
        {PHASES.map((p, idx) => {
          ...
          const width = ((b - a + 1) / 21) * 100;
          return (
            <div key={p.id} className={`tl-gantt-row ${idx < shown ? 'in' : ''}`}>
              <div className="tl-gantt-row-meta">
                <span className="label">Phase 0{p.id}</span>
                <span className="tl-gantt-row-name">{p.name}</span>
              </div>
              <div className="tl-gantt-track">
                <div className={`tl-gantt-bar band-${p.id}`} style={{ left: `${left}%`, width: `${width}%` }}>
                  <span className="num">D{a}–D{b}</span>
                </div>
                {MILESTONES.filter((m) => m.phase === p.id).map((m, j) => (
                  <div key={m.day} className={`tl-gantt-ms ${shown > PHASES.length + j ? 'in' : ''}`}
                    style={{ left: `${((m.day - 0.5) / 21) * 100}%` }} title={`Day ${m.day} · ${m.label}`}>
      ...
      <div className="tl-gantt-today" style={{ left: `calc(var(--meta-w) + ((${TODAY} - 0.5) / 21) * (100% - var(--meta-w)))` }}>
        <div className="tl-gantt-today-line"/>
        <div className="tl-gantt-today-tag"><span className="num">Day {TODAY}</span> · today</div>
      </div>
    </div>
  );
}
...
function TimelineLegend() {
  return (
    <div className="tl-legend">
      {PHASES.map((p) => (
        <div key={p.id} className="tl-legend-item">
          <span className={`tl-legend-dot band-${p.id}`}/>
          <span className="label">Phase 0{p.id}</span>
          <span className="tl-legend-name">{p.name}</span>
          <span className="tl-legend-range num">Days {p.range[0]}–{p.range[1]}</span>
        </div>
      ))}
    </div>
  );
...
Object.assign(window, { TimelineCalendar, TimelineBeam, TimelineGantt, TimelineLegend });
